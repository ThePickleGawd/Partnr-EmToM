...
class LLMPlanner(Planner):
    ...
    def _maybe_get_vlm_image(
        self, observations: Dict[str, Any]
    ) -> Optional[Tuple[str, Image.Image]]:
        """
        Try to extract an RGB observation for the primary agent and convert it into a
        data URL expected by VLM backends, returning both the data URL and PIL image.
        """
        if observations is None:
            return None

        agent_uid = str(self._agents[0].uid) if len(self._agents) > 0 else ""
        candidate_image = None

        # Prefer the current agent's RGB view; otherwise fall back to any RGB view.
        for key, value in observations.items():
            if "rgb" in key and agent_uid in key:
                candidate_image = value
                break
        ...
        try:
            if torch.is_tensor(candidate_image):
                image_arr = candidate_image.detach().cpu().numpy()
            else:
                image_arr = np.asarray(candidate_image)

            if image_arr.ndim == 4:
                image_arr = image_arr[0]
            if image_arr.ndim == 3 and image_arr.shape[0] in (1, 3):
            ...
            return pil_image_to_data_url(image_pil), image_pil
        except Exception as exc:
            print(f"Warning: could not prepare VLM image input: {exc}")
            return None

    def _save_prompt_io(
        self, image_pil: Image.Image, prompt_text: str, response: str
    ) -> None:
        """
        Persist the prompt-image-response triple for debugging.
        """
        try:
            dataset_file = (
                self.env_interface.conf.habitat.dataset.data_path.split("/")[-1]
            )
            results_dir = self.env_interface.conf.paths.results_dir
            base_dir = os.path.join(results_dir, dataset_file, "llm_visual_inputs")
            os.makedirs(base_dir, exist_ok=True)

            # episode id (best-effort)
            episode_id = None
            try:
                episode_id = (
            ...
            prefix = f"step_{self.replanning_count:05d}"
            if episode_id is not None:
                prefix += f"_episode_{episode_id}"

            image_path = os.path.join(base_dir, f"{prefix}.png")
            text_path = os.path.join(base_dir, f"{prefix}.txt")
            ...
            print(f"Warning: could not save prompt/image pair: {exc}")
    ...
    def replan(
        ...
        """
        # Generate response
        prompt_input: Union[str, List[Tuple[str, str]]] = self.curr_prompt
        image_payload: Optional[Tuple[str, Image.Image]] = None
        use_image_input = getattr(self.planner_config.llm, "use_image_input", False)
        if use_image_input and isinstance(self.llm, (VLMHFModel, OpenAIChat)):
            vlm_image = self._maybe_get_vlm_image(observations)
            if vlm_image is not None:
                image_payload = vlm_image
                prompt_input = [("text", self.curr_prompt), ("image", vlm_image[0])]
        ...
        if getattr(self.planner_config.llm, "save_prompt_images", False):
            if image_payload is not None:
                self._save_prompt_io(image_payload[1], self.curr_prompt, llm_response)
    ...
    def get_next_action(
        ...
        post_action_message_responses = self._consume_agent_messages()
        if len(post_action_message_responses) > 0:
            print_str += self._add_responses_to_prompt(post_action_message_responses)
        ...
        def _append_response(storage, agent_uid, text):
            if text is None or text == "":
                return
            if agent_uid in storage and storage[agent_uid]:
                storage[agent_uid] += f"\n{text}"
            else:
                storage[agent_uid] = text

        combined_responses: Dict[int, str] = {}
        for agent_uid, text in message_responses.items():
            _append_response(combined_responses, agent_uid, text)
        for agent_uid, text in post_action_message_responses.items():
            _append_response(combined_responses, agent_uid, text)
        for agent_uid, text in responses.items():
            if text is None:
                continue
            if text == "":
                combined_responses.setdefault(agent_uid, "")
            else:
                _append_response(combined_responses, agent_uid, text)
